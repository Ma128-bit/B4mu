...
void MiniAnaB2Mu2K::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    if(isMc){
        ...
        for(edm::View<reco::GenParticle>::const_iterator gp=genParticles->begin(); gp!=genParticles->end(), j<ngenP; ++gp , ++j){
            ...
            if( fabs(gp->pdgId())==13  || fabs(gp->pdgId())==22  || fabs(gp->pdgId())==511 || fabs(gp->pdgId())==531 || fabs(gp->pdgId())==513 || fabs(gp->pdgId())==533 || fabs(gp->pdgId())==333 || fabs(gp->pdgId())==443 || fabs(gp->pdgId())==321 || fabs(gp->pdgId())==313 || fabs(granddaughter->pdgId())==211) { //mu gamma B0 B0s B*0 B*0s Φ J/Psi K+ K*
