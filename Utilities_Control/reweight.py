    ...
    if compute == True:
        ...
        for name in hnames:
            if "h_ratio_" in name:
                ...
                addw(base_path+"_rw.root", "FlightDistBS_SV_Significance" if name == "0" else "Quadruplet_Eta", histograms[hnames.index("h_ratio_" + name)], name)
        file.Close()
        if new_bdtw:
            change_bdt_w(base_path+"_rw.root")
