...
if __name__ == "__main__":
    ...
    for var in var_dict2:
        ...
        canvas.DefTLine(Color=1, Orientation=0, X=cuts[0],  label=f"Left Cut = {round(cuts[0],2)}")
        canvas.DefTLine(Color=8, Orientation=0, X=cuts[1],  label=f"Right Cut = {round(cuts[1],2)}")
        canvas.MakeLegend()
